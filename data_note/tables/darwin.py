...
def make_table2_rows(context):
    ...
    if asm_type == "hap_asm":
        ...
        if context.get("hap1_supernumerary_chromosomes") or context.get("hap2_supernumerary_chromosomes"):
            rows.insert(
                ...
                [
                    ...
                    safe_str(context.get("hap1_supernumerary_chromosomes")),
                    safe_str(context.get("hap2_supernumerary_chromosomes")),
                ],
            )
        ...
        if context.get("supernumerary_chromosomes"):
            rows.insert(
                -1,
                ["**Supernumerary chromosomes**", safe_str(context.get("supernumerary_chromosomes"))],
            )
...
def make_table4_rows(context: dict) -> dict:
    ...
    def native_cell_local(value) -> str:
    ...
    def add_row(measure, value, benchmark):
        ...
        native_rows.append(
            {
                "measure": native_cell_local(measure),
                "value": native_cell_local(value),
                "benchmark": native_cell_local(benchmark),
            }
        )
    ...
    notes = (
        "The EBP summary uses log10(Contig N50); chromosome-level (C) or log10(Scaffold N50); Q (Merqury QV). "
        "BUSCO: C=complete; S=single-copy; D=duplicated; F=fragmented; M=missing; n=orthologues"
    )
