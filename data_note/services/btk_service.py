...
from ..fetch_btk_info import (
    build_btk_urls,
    fetch_and_parse_summary,
...
class BtkService:
    ...
    accession_resolver: Callable[[str], str | None] | None = None
    ...
    def _populate_primary_context(self, assembly_selection: AssemblySelection, btk_summary: BtkSummary) -> None:
        ...
        btk_accession = self._resolve_accession(prim_accession)
    ...
    def _populate_haplotype_context(self, assembly_selection: AssemblySelection, btk_summary: BtkSummary) -> None:
        ...
        if assembly_selection.hap2 is not None:
            ...
            hap2_btk_accession = self._resolve_accession(hap2_accession)
            try:
                btk_summary.hap2 = self._build_record(hap2_btk_accession, prefix="hap2_")
            except Exception as exc:
                logger.warning("BTK data missing or failed for hap2 %s: %s", hap2_btk_accession, exc)
        else:
    ...
    def _build_record(self, accession: str, *, prefix: str = "") -> BtkAssemblyRecord:
        ...
        summary_fields[f"{prefix}btk_accession"] = accession
    ...
    def _resolve_accession(self, accession: str) -> str:
        ...
        if resolver is not None:
            try:
                resolved = resolver(accession) or accession
            except Exception as exc:
                logger.warning("BTK accession resolution failed for %s: %s", accession, exc)
                return accession
        else:
            override_getter = getattr(self.taxonomy_mapper_module, "get_btk_accession_override", None)
            override = override_getter(accession) if callable(override_getter) else None
            resolved = override.get("accession") if override else accession

        if resolved != accession:
            logger.info("Using BlobToolKit dataset %s for assembly %s.", resolved, accession)
        return resolved
