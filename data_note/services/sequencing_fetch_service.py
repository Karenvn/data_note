...
class SequencingFetchService:
    ...
    def fetch_read_runs_for_bioproject(self, bioproject: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Filereport failed for {bioproject}: HTTP {response.status_code}")
                return []
            ...
            print(f"Filereport request failed for {bioproject}: {exc}")
            return []
    ...
    def fetch_runinfo_rows_for_accession(self, accession: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            print(f"RunInfo request failed for {accession}: {exc}")
            return []
    ...
    def fetch_sra_summary_rows_for_accession(self, accession: str) -> list[dict[str, Any]]:
        ...
        for term in search_terms:
            try:
                ...
                print(f"SRA E-utilities request failed for {accession} ({term}): {exc}")
    ...
    def fetch_for_bioprojects(self, bioprojects: list[str]) -> pd.DataFrame:
        ...
        for accession in bioprojects:
            ...
            if not accession_rows:
                print(f"No SRA RunInfo rows found for {accession}; trying NCBI E-utilities summary.")
                accession_rows = self.fetch_sra_summary_rows_for_accession(accession)
            if not accession_rows:
                print(f"No SRA summary rows found for {accession}; falling back to ENA filereport.")
    ...
    def fetch_experiment_protocol(self, experiment_accession: str) -> str:
        ...
        try:
            ...
            print(f"Experiment protocol fetch failed for {experiment_accession}: {exc}")
        return ""
